...
<div class="flex gap-2">
  <Tooltip.Root>
    <Tooltip.Trigger asChild let:builder={tooltipBuilder} class="outline-none">
      <Button onclick={handleEdit} builders={[tooltipBuilder]} variant="ghost" size="icon">
        <svg
          class="pointer-events-none size-5 text-neutral-600 dark:text-neutral-500"
  ...
  <Tooltip.Root>
    <Tooltip.Trigger asChild let:builder={tooltipBuilder} class="outline-none">
      <Button
        onclick={() => toggleDeleteDialog(id)}
        builders={[tooltipBuilder]}
        variant="ghost"
        size="icon"
      >
        <svg
          class="text-red-600 pointer-events-none size-5 dark:text-red-400/50"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 256 256"
