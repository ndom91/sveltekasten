<script lang="ts">
  ...
  const handleToggleCard = (e) => {
    console.log("toggling Card", e)
    handleToggleCardOpen()
  }
</script>

<div class="flex absolute top-2 right-2 flex-col">
  <Button variant="ghost" size="icon" class="" onclick={handleToggleCard}>
  ...
  <DropdownMenu.Root>
    ...
    <DropdownMenu.Content class="">
      <DropdownMenu.Group>
        <DropdownMenu.Item>
          <a href={url} target="_blank" class="grid grid-cols-[28px_1fr]">
        ...
        <DropdownMenu.Item class="grid grid-cols-[28px_1fr]" onclick={() => handleMarkAsUnread()}>
          <svg
            class="size-[1.2rem]"
            data-slot="icon"
            fill="none"
