...
<div class="flex absolute top-0 right-0 flex-col justify-start mt-2 h-full">
  ...
  <button class="flex justify-center p-3" popovertarget={`feed-menu-${id}`} style={`anchor-name: --a-${id}`}>
    <svg
      class="size-6"
      data-slot="icon"
      fill="none"
  ...
  <div
    id={`feed-menu-${id}`}
    popover="auto"
    class="p-2 space-y-3 rounded-md border border-neutral-200 dark:border-neutral-800"
    transition:fade
    style={`
      position: absolute;
